...
// Pokemon names for Gens 1-5 (649 total)
...
const getGeneration = (dex: number): number => {
  if (dex <= 151) return 1
  if (dex <= 251) return 2
  if (dex <= 386) return 3
  if (dex <= 493) return 4
...
export const pokemonList: Pokemon[] = Object.entries(pokemonNames).map(([dexStr, name]) => {
  const dexNumber = Number(dexStr)
  return {
    id: name.toLowerCase().replace(/[^a-z0-9]/g, '-'),
    name,
    dexNumber,
    generation: getGeneration(dexNumber),
    sprites: makeSprites(dexNumber)
  }
})
